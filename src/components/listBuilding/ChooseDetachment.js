...
export default function ChooseDetachment() {
    
    const navigate = useNavigate();
    const location = useLocation();

    const chosenFaction=localStorage.getItem("chosenFactionID")
                        ...
                        <div className='w-full h-1/6 grid place-items-center'>
                            <div className='w-2/3 h-2/3 px-2 py-1 rounded-lg bg-cam-blue flex flex-col justify-center items-center'>
                                <Button
                                    ...
                                    handleAction={() => {
                                        localStorage.setItem("chosenDetachmentID", doc.id)
                                        localStorage.setItem("chosenDetachmentName", doc.data().name)
                                        navigate(location.pathname+'/choose-size')}}
