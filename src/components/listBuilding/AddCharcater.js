...
export default function AddCharacter() {
    ...
    const [data, isLoading, isError] = useCollection( //Working Properly
        query(
            ...
            where('keywords.unit-keywords', 'array-contains', 'CHARACTER')
        )
    )
                    ...
                    {data.docs.map(doc => (
                        <div className='w-full h-1/6 grid place-items-center'>
                            <div className='w-2/3 h-2/3 px-2 py-1 rounded-lg bg-cam-blue flex flex-col justify-center items-center'>
                                <Button
                                    title={<div className='flex justify-between'> <div>{doc.data().name.toUpperCase()}</div> <div>{doc.data().points['full-squad']['points'] + ' Points'}</div></div>}
                                    handleAction={() => {
                                        localStorage.setItem("chosenCharacter", doc.id)
                                        localStorage.setItem("characterPoints", doc.data().points['full-squad']['points'])
